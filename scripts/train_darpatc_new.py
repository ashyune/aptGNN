...
from data_process_train_new import MyDataset
from data_process_test_new import MyDatasetA
...
MIN_WINDOW_EDGES = 5000    # floor: a window's own edge_index needs enough
                            # edges for the relation-aware convolution to
                            # have real neighbourhoods to aggregate over.
                            # If --num_windows would make windows thinner
                            # than this, we silently reduce the effective
                            # window count instead of creating message-
                            # passing-starved windows.
...
class RGCNMemNet(torch.nn.Module):
    """Two-layer, EDGE-TYPE-AWARE relational-graph classifier + a persistent,
    GRU-updated GLOBAL memory vector carried across time windows.

    This is SAGEMemNet from the original temporal-memory implementation
    with exactly one structural change: conv1/conv2 (torch_geometric.nn
    SAGEConv, which aggregates every edge -- READ, WRITE, EXECUTE, SEND,
    RECV, CONNECT, FORK, ... -- through one shared message function) are
    replaced by RGCNConv (torch_geometric.nn), which learns a SEPARATE
    per-relation transformation and message-passes each edge through the
    weight matrix that matches its edge_type. Every other component --
    the readout, the GRU memory cell, the context projection/gate, the
    dropout, the log_softmax head -- is untouched, byte-for-byte the same
    tensor shapes and same order of operations as before.
    ...
    logic, same convention as before. edge_type is a LongTensor aligned
    1:1 with the columns of edge_index (see data_process_train_new.py /
    data_process_test_new.py) -- NeighborLoader carries it through
    sampling automatically since it is sized like an edge attribute.

    num_relations: number of distinct edge types in this scene (returned
    by MyDataset / MyDatasetA as num_relations). Must match between a
    checkpoint and whatever data it is later loaded against -- both sides
    derive it from the same feature.txt, so this holds automatically as
    long as the scene isn't changed mid-run.

    num_bases: optional basis-decomposition size for RGCNConv, passed
    straight through. None (the RGCNConv default) means a full,
    undecomposed per-relation weight -- fine for the DARPA TC scenes here
    (a handful to a few dozen edge types). Exposed as a constructor arg in
    case a future scene has a much larger relation vocabulary and basis
    sharing becomes worth trading off against per-relation specificity.
    """
    def __init__(self, in_channels, out_channels, num_relations, mem_dim=MEM_DIM, num_bases=None):
        super().__init__()
...
def make_loader(data, mask, b_size, shuffle=False):
    ...
    edge_type needs no special handling here: NeighborLoader/NeighborSampler
    slice every edge-level tensor attribute (anything sized like
    edge_index's second dimension) the same way they slice edge_index
    itself, so each sampled batch's batch.edge_type is already correctly
...
def validate(args, b_size, thre, graphId, device, num_relations):
    ...
    num_relations is passed in from train_pro() (rather than re-derived
    from MyDatasetA's own return value) so the reconstructed model
    architecture is guaranteed identical to the one the checkpoints were
    trained with, even though MyDatasetA(test) also independently computes
    the same number by reading the same feature.txt.
    """
    ...
    data, feature_num, label_num, test_num_relations, adj, adj2, nodeA, _nodeA, _neighbour = \
        MyDatasetA(path, 0)
    assert test_num_relations == num_relations, (
        f'Relation-vocabulary mismatch between train ({num_relations}) and '
        f'test ({test_num_relations}) scenes -- feature.txt was likely '
        f'regenerated between the two MyDataset*/MyDatasetA calls.'
    )
    ...
    print(data)
    model = RGCNMemNet(feature_num, label_num, num_relations).to(device)
    eps = 1e-10
...
def train_pro(args, b_size, thre, num_windows=NUM_WINDOWS):
    ...
    state forward from window to window. Returns (graphId, device,
    num_relations) for validate().
    """
    ...
    show(f'feature {feature_num}; label {label_num}; '
         f'{num_relations} relations; {len(windows)} windows (requested {num_windows})')
    ...
    return graphId, device, num_relations
...
def main():
    ...
    assert args.model in ['RGCN', 'SAGE']
    ...
    while True:
        graphId, device, num_relations = train_pro(args, b_size, thre, num_windows=args.num_windows)
        flag = validate(args, b_size, thre, graphId, device, num_relations)
        if flag == 1:
            break
