...
MEM_DIM = 64  # value used in train_darpatc_new.py
...
class RGCNMemNet(torch.nn.Module):
    """Same architecture as train_darpatc_new.py's RGCNMemNet -- duplicated
    here because this script is standalone and doesn't import from
    train_darpatc_new.py. See that file for the full design rationale
    (relation-aware conv1/conv2 + untouched GRU memory / readout / context
    fusion).
    """
...
def run_test(model, data, b_size, device, thre, state):
    ...
    every batch (same convention as train_darpatc_new.py's test()/
    final_test(): no intra-eval drift, order-independent result).
...
def main():
    ...
    # --- Data loading ---
    data, feature_num, label_num, num_relations, adj, adj2, nodeA, _nodeA, _neighbour = \
