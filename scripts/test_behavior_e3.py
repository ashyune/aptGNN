"""Extension 3 scoring: behavior + window-statistic surprise from a
frozen EnrichedBehaviorNet checkpoint -- produces scores_windowed.txt.

Protocol is IDENTICAL to test_behavior.py (node-level, no neighborhood
...
output evaluated by evaluate_windowed.py unchanged). Only the per-node
per-window score differs: combined_nll = profile NLL + unweighted mean
of the three feature-bin NLLs (the weighting is locked in
train_behavior_e3.feature_nll; this file adds no knob for it).
"""
...
from window_features import FEATURE_BIN_SIZES, build_enriched_dataset
from train_behavior import type_onehot
from train_behavior_e3 import (EnrichedBehaviorNet, combined_nll,
                               make_writeback_e3)
...
def score_dataset(model, dataset, node_memory, num_types):
    """Mirrors test_behavior.score_dataset with the combined E3 score."""
    score_by_gid = {}
    for window_idx, data in enumerate(dataset):
        ...
        scores = combined_nll(out, data.x, feat_out,
                              data.feat_bins).cpu().tolist()
...
def main():
    parser = argparse.ArgumentParser(
        description='Extension 3 scoring from a frozen EnrichedBehaviorNet '
                    'checkpoint.'
    )
    parser.add_argument('--scene', type=str, default='cadets',
                        choices=['cadets', 'trace', 'theia', 'fivedirections'])
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Defaults to ../models/behavior_<scene>_e3/best_model.pt')
    ...
    parser.add_argument('--relational', action='store_true',
                        help='Must match the frozen training config '
                             '(state_dict load fails loudly on a mismatch).')
    ...
    parser.add_argument('--out-dir', type=str, default=None,
                        help='Defaults to ../models/behavior_<scene>_e3/eval/')
    ...
    checkpoint_path = args.checkpoint \
        or f'../models/behavior_{args.scene}_e3/best_model.pt'
    out_dir = args.out_dir or f'../models/behavior_{args.scene}_e3/eval'
    ...
    show(f'Scene: {args.scene} | operating point = train quantile '
         f'{args.flag_quantile} | feature bins {FEATURE_BIN_SIZES}')
    ...
    show('Building enriched windowed train dataset (operating-point '
         'calibration)')
    train_dataset = build_enriched_dataset(
        train_path, node_vocab, feature_map, label_map, args.window_size)
    show(f'{len(train_dataset)} train windows')
    calib_memory = NodeMemory(len(node_vocab), model.memory_dim,
                              args.decay_rate,
    ...
    show('Building enriched windowed test dataset')
    test_dataset = build_enriched_dataset(
