...
def MyDatasetA(path, model):
    ...
    # Captured BEFORE feature_num is doubled below -- this is the number of
    # distinct edge types (READ/WRITE/EXECUTE/...) in feature.txt, i.e. the
    # same vocabulary the training scene used to build its relation ids.
    # Passed back to the caller so the relation-aware convolution can be
    # constructed with the correct num_relations (must match the trained
    # checkpoint's architecture).
    num_relations = feature_num
    ...
    with open('groundtruth_nodeId.txt', 'w') as fw, \
        ...
        for line in f:
            ...
            edge_t.append(temp[4])   # relation id, aligned 1:1 with edge_s/edge_e
    ...
    return data, feature_num, label_num, num_relations, adj, adj2, nodeA, _nodeA, _neighbour
